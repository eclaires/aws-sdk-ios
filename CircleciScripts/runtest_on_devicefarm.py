...
def runtest(project, scheme, test, hostApp, sdkfolder, testfilter, displayname):    
    print("################################### {0} ###################################".format(test))
    ...
    response = devicefarm_client.schedule_run(
        ...
        name='{0}-{1}'.format(displayname, tag),
        test={
            'type': 'XCTEST',
...
for project in projectlist:
    ...
    for test in testlist:
        ...
        displayname = None 
        if "displayname" in test:
            displayname = test["displayname"]


        if 0 != runtest(projectpath , scheme, testname, hostAppName, sdkfolder, testfilter, displayname):
